...
std::vector<SceneData> Parser::parse(const std::string& filepath) {
    ...
    std::string aggregator_type  = "list";
    std::string bvh_split_method = "equal_counts";
    int    bvh_max_prims         = 4;
    ...
    while (pos < xml.size()) {
        ...
        if (name == "lookat") {
            ...
            aggregator_type  = extractAttr(tag, "type");
            std::string sm   = extractAttr(tag, "split_method");
            if (!sm.empty()) bvh_split_method = sm;
            bvh_max_prims    = extractIntOpt(tag, "max_prims_per_node", 4);
        }
        else if (name == "camera") {
        ...
        // Tags dentro do mundo
            ...
            if (name == "background") {
                ...
                // Reescala cores de background: 'normalized' indica se vem em [0,1] ou [0,255].
            ...
            // Objetos
                ...
                // Malha de triangulos (proj07)
                    ...
                    if (!fname.empty()) {
                        ...
                        // Sem normal_indices mas com normais por vertice: usa vertex_indices.
            ...
            // Fim do mundo
                ...
                data.aggregator_type  = aggregator_type;
                data.bvh_split_method = bvh_split_method;
                data.bvh_max_prims    = bvh_max_prims;
